...
import { DelEnumCategory, GetEnumCategorys, SaveEnumCategory, type SaveEnumCategoryParamsType } from '@/api/sys/enum';
...
const [EnumTableComp, enumTableMethods] = useTable({
    columns: [
        ...
        { prop: 'title', label: '标题' },
        {
            prop: 'details', label: '详情', formatter: (row, details) => {
...
const [SaveEnumDialogComp, saveEnumDialogMethods] = useDialogForm<SaveEnumCategoryParamsType>({
    labelPosition: 'top',
    formSchemas: [
        { field: 'name', component: 'Input', colProps: { span: 12 }, label: '名称（英）', componentProps: { placeholder: '请输入英文名称' } },
        { field: 'title', component: 'Input', colProps: { span: 12 }, label: '标题（中）', componentProps: { placeholder: '请输入中文标题' } },
        {
            field: 'details', component: 'EditableTable', label: '详情', componentProps: () => {
                return {
                    columns: [
                        {
                            prop: 'name', label: '名称', editConfig: {
                                component: 'Input'
                            }
                        }, {
                            prop: 'value', label: '值', editConfig: {
                                component: 'Input'
                            }
                        }, {
                            prop: 'description', label: '备注', editConfig: {
                                component: 'Input'
                            }
                        }
                    ]
        ...
        { field: 'description', component: 'Input', label: '备注', componentProps: { type: 'textarea', autosize: { minRows: 5, maxRows: 5 } } },
    ],
    title: `枚举管理`,
...
async function edit(row) {
    return await saveEnumDialogMethods.open({
        ...
        name: row.name,
        title: row.title,
        details: row.details.map((detail) => pick(detail, ['id', 'name', 'value', 'description'])),
