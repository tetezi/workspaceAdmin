...
const metaRef = ref();
const containerComponentOptions = Array.from(containerComponentMap.keys()).map(
  (key) => ({
    label: key,
    value: key,
  })
);
const inputComponentOptions = Array.from(inputComponentMap.keys()).map(
  (key) => ({
    label: key,
    value: key,
  })
);
const displayComponentOptions = Array.from(displayComponentMap.keys()).map(
  (key) => ({
    label: key,
    value: key,
  })
...
const [DialogFormComp, dialogFormMethods] =
  useDialogForm<DynamicFormSchemaType>({
    title: `表单子项配置`,
    labelPosition: "top",
    height: "80vh",
    width: "80vw",
    formSchemas: [
      ...
      },
      {
        ...
        colProps: { span: 4 },
        tip: "可用符号.深度映射，如：a.b，但请确保a已声明而非“a is not defined”",
      },
      {
        field: "colProps.span",
        label: "分栏span",
        component: "InputNumber",
        componentProps: { min: 1, max: 24 },
        colProps: { span: 4 },
      },
      {
        field: "category",
          ...
          if (formValue.category === "Container") {
            ...
            return {
              options: [
                ...
                { label: "EnumSelect", value: "EnumSelect" },
              ],
            };
      ...
      },
      {
        ...
        componentProps: ({ formValue }) => ({
          title: "输入型组件配置",
          defaultValue: Boolean(
            formValue.labelShow.value !== true ||
              formValue.labelRender ||
              formValue.labelWidth?.value ||
              formValue.labelWidth?.code
          ),
        }),
        ifShow: ({ formValue }) => {
          return formValue.category === "Input";
        ...
        children: [
          ...
          },
          {
            ...
            tip: `默认参考表单字段label宽度配置，请返回CSSProperties['width']`,
            ifShow: ({ formValue }) => {
      ...
      },
      {
        ...
        componentProps: ({ formValue }) => {
          return {
            title: "组件高级配置",
            defaultValue: Boolean(
              formValue.componentProps ||
                formValue.componentStyle ||
                formValue.componentSlot ||
                formValue.componentRender
            ),
          };
        },
        children: [
          ...
          },
          {
            ...
            tip: "默认空配置，请返回Recordable<() => VNodeChild>",
            colProps: { span: 12 },
          },
          {
            ...
            tip: "警告！此设置将会覆盖原组件渲染逻辑，请返回VNodeChild",
            colProps: { span: 12 },
          },
        ],
      },
