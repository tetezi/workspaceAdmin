...
import { unref, computed, watch, ref, markRaw, watchEffect } from 'vue';
import PreinstallFormSchemas from './PreinstallFormSchemas.vue';
import FormSchemaConfig from './FormSchemaConfig.vue';
...
watch([isOk,modelValue], (val) => {
    if (unref(isOk)) {
        if(unref(modelValue)!== unref(formMethods.getSchemas)){ 
        formMethods.setSchemas(unref(modelValue) || []) 
        }
    }
})
watch(()=>(unref(isOk)&&unref(formMethods.getSchemas)), (val) => {
    if (unref(isOk)) {
        if(unref(modelValue)!== unref(formMethods.getSchemas)){ 
        modelValue.value = unref(formMethods.getSchemas) 
        }
    }
})
</script>
